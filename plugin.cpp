...
namespace SkyrimScripting::Bind {
    ...
    std::unordered_map<std::string, bool> LiterallyEveryQuestEditorID;
    ...
    void RegisterQuest(const char* editorID) {
        if (!editorID) return;
        std::string questEditorID{editorID};
        if (questEditorID.empty()) return;
        LowerCase(questEditorID);
        LiterallyEveryQuestEditorID.insert(std::pair{questEditorID, true});
        _Log_("Registered quest '{}'", questEditorID);
    }
    void LoadLiterallyEveryQuestEditorID() {
        auto  bufferOfMissingFormIDs = 10000;
        auto  missedFormIDs          = 0;
        auto  nextFormID             = 0xff000aaa;
        auto* nextForm               = RE::TESForm::LookupByID(nextFormID);
        while (nextForm && missedFormIDs < bufferOfMissingFormIDs) {
            nextFormID++;
            nextForm = RE::TESForm::LookupByID(nextFormID);
            // if (nextForm && nextForm->GetFormType() == RE::FormType::Quest)
            //     RegisterQuest(nextForm->GetFormEditorID());
            if (nextForm) {
                missedFormIDs = 0;
                RegisterQuest(nextForm->GetFormEditorID());
                _Log_("{:x} [{}]", nextFormID, nextForm->GetFormEditorID());
            } else missedFormIDs++;
        }
        _Log_("Searched {} form IDs for quests", nextFormID - 0xff000aaa);
    }
    bool QuestExists(const std::string& editorID) {
        std::string lowercaseEditorID{editorID};
        LowerCase(lowercaseEditorID);
        if (LiterallyEveryQuestEditorID.empty()) LoadLiterallyEveryQuestEditorID();
        _Log_("Checking if quest '{}' exists", lowercaseEditorID);
        return LiterallyEveryQuestEditorID.find(lowercaseEditorID) !=
               LiterallyEveryQuestEditorID.end();
    }
    RE::TESForm* LookupFormID(RE::FormID formID) {
        ...
        if (form) return form;
            ...
            _Log_(
                "[BIND] Error [{}:{}] ({}) Form ID '{:x}' does not exist", FilePath, LineNumber,
                ScriptName, formID
            );
        return {};
    }
    RE::TESForm* LookupEditorID(const std::string& editorID) {
        ...
        if (form) return form;
            ...
            _Log_(
                "[BIND] Error [{}:{}] ({}) Form Editor ID '{}' does not exist (Are you using po3 "
                "Tweaks?)",
    ...
    void Bind_Form(RE::TESForm* form) {
        ...
        if (FormHasScriptAttached(handle, ScriptName)) {
            _Log_(
                "[BIND] Script {} already attached to form {:x} ({})", ScriptName,
            ...
            if (reference)
                _Log_(
                    "[BIND] Bound script {} to form {:x} {} (base type: {:x} {})", ScriptName,
                    form->GetFormID(), form->GetName(), reference->GetBaseObject()->GetFormID(),
                    reference->GetBaseObject()->GetName()
                );
            else
                _Log_(
                    "[BIND] Bound script {} to form {:x} {}", ScriptName, form->GetFormID(),
                    form->GetName()
                );
        }
    }
    RE::TESObjectREFR* GenerateObject(RE::TESForm* baseForm) {
        RE::TESObjectREFR* objectRef = nullptr;
        if (!baseForm)
            baseForm =
                DefaultBaseFormForCreatingObjects;  // By default, simply puts a fork next to the
        // WEMerchantChest in the WEMerchantChests cell.
        // Forking awesome.
        _Log_(
            "Calling PlaceObjectAtMe with location {:x} and base form {:x}",
            LocationForPlacingObjects->GetFormID(), baseForm->GetFormID()
        );
        auto niPointer = LocationForPlacingObjects->PlaceObjectAtMe(
            skyrim_cast<RE::TESBoundObject*, RE::TESForm>(baseForm), false
        );
        if (niPointer) {
            auto* newObjectRef = niPointer.get();
            _Log_(
                "[BIND] Generated object {:x} (base type: {:x} {}) {}", newObjectRef->GetFormID(),
                baseForm->GetFormID(), baseForm->GetName(), baseForm->GetFormEditorID()
            );
            return newObjectRef;
        } else {
            _Log_(
                "[BIND] Error [{}:{}] ({}) Could not generate object ({:x}, {})", FilePath,
                LineNumber, ScriptName, baseForm->GetFormID(), baseForm->GetName()
            );
            return nullptr;
        }
    }
    void Bind_GeneratedObject(RE::TESForm* baseForm = nullptr) {
        if (!baseForm) {
            _Log_("[BIND] Bind_GeneratedObject called with nullptr (no base form)");
            return;
        }
        Bind_Form(baseForm);
    }
    void Bind_GeneratedObject_WithPersistenceID(
        RE::TESForm* baseForm, const std::string& persistenceID = ""
    ) {
        if (QuestExists(persistenceID)) {
            _Log_("Quest with Editor ID '{}' already exists", persistenceID);
            return;
        }
        auto* quest = RE::IFormFactory::GetConcreteFormFactoryByType<RE::TESQuest>()->Create();
        quest->SetFormEditorID(persistenceID.c_str());
        quest->Start();
        _Log_(
            "Created new quest {:x} with editor id '{}'", quest->GetFormID(),
            quest->GetFormEditorID()
        );
        RegisterQuest(quest->GetFormEditorID());
        auto* object = GenerateObject(baseForm);
        if (!object) {
            _Log_(
                "[BIND] Error [{}:{}] ({}) Could not generate object ({:x}, {})", FilePath,
                LineNumber, ScriptName, baseForm->GetFormID(), baseForm->GetName()
        ...
        Bind_GeneratedObject(object);
    }
    void Bind_GeneratedObject_BaseEditorID(
        const std::string& baseEditorId, const std::string& persistenceID = ""
    ) {
        auto* form = LookupEditorID(baseEditorId);
        if (form) {
            if (!persistenceID.empty()) Bind_GeneratedObject_WithPersistenceID(form, persistenceID);
            else Bind_GeneratedObject(GenerateObject(form));
        } else
            _Log_(
                "[BIND] Error [{}:{}] ({}) Base Editor ID '{}' does not exist (Are you using po3 "
                "Tweaks?)",
                FilePath, LineNumber, ScriptName, baseEditorId
            );
    }
    void Bind_GeneratedObject_BaseFormID(
        RE::FormID baseFormID, const std::string& persistenceID = ""
    ) {
        ...
        if (form) {
            ...
            _Log_(
                "[BIND] Error [{}:{}] ({}) Base Form ID '{:x}' does not exist", FilePath,
                LineNumber, ScriptName, baseFormID
            );
    }
    void Bind_GeneratedQuest(const std::string& editorID = "") {
        RE::TESQuest* quest = nullptr;
        if (!editorID.empty() && QuestExists(editorID)) {
            _Log_("Quest with Editor ID '{}' already exists", editorID);
            return;
        }
        if (!quest) {
            ...
            _Log_(
                "[BIND] Quest already generated {:x} {}", quest->GetFormID(),
                quest->GetFormEditorID()
            );
        }
        Bind_Form(quest);
    ...
    bool IsUnderstoodScriptParentType(std::string parentTypeName) {
        ...
        if (parentTypeName == "quest" || parentTypeName == "actor" ||
            parentTypeName == "objectreference")
            return true;
        return false;
    }
    void AutoBindBasedOnScriptExtends() {
        ...
        if (parentName.empty()) {
            _Log_(
                "[BIND] Error [{}:{}] ({}) Cannot auto-bind to a script which does not `extends` "
                "anything",
        ...
        if (parentName == "quest") Bind_GeneratedQuest();
            ...
            _Log_(
                "[BIND] Error [{}:{}] ({}) No default BIND behavior available for script which "
                "`extends` {}",
    ...
    void ProcessBindingLine(std::string line) {
        ...
        if (ScriptName.empty() || ScriptName.starts_with('#') || ScriptName.starts_with("//"))
            return;
        _Log_("[BIND] [{}:{}] {}", FilePath, LineNumber, line);
        if (!vm->TypeIsValid(ScriptName)) {
            _Log_(
                "[BIND] Error [{}:{}] Script '{}' does not exist", FilePath, LineNumber, ScriptName
            );
            return;
        }
        ...
        if (bindTarget.empty()) {
            ...
            if (commaIndex != std::string::npos) {
                ...
                afterComma  = bindTarget.substr(
                    commaIndex + 1, bindTarget.size() - commaIndex - 2
                );  // -2 to remove the closing parenthesis
                afterComma.erase(0, afterComma.find_first_not_of(' '));
            } else {
                beforeComma = bindTarget.substr(
                    10, bindTarget.size() - 11
                );  // -11 to remove the "0x" and the closing parenthesis
                afterComma = "";
            }
            ...
            if (commaIndex != std::string::npos) {
                ...
                beforeComma = bindTarget.substr(
                    8, bindTarget.size() - 9
                );  // -9 to remove the closing parenthesis
                afterComma = "";
            }
    ...
    void RunBindings() {
        ...
        LiterallyEveryQuestEditorID.clear();
    }
